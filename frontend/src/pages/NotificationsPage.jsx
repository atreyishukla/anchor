...
const NotificationsPage = () => {
	...
	const renderNotificationContent = (notification) => {
		switch (notification.type) {
			case "like":
				return (
					<span>
						<strong>{notification.relatedUser.name}</strong> voted your post
					</span>
				);
			case "commented":
				return (
	...
	const renderRelatedPost = (relatedPost) => {
		...
		// Fallback to _id if postId is undefined
		const postId = relatedPost.postId || relatedPost._id;
	
		// Check if postId is valid to avoid navigating to /post/undefined
		if (!postId) return null;
	
		return (
			<Link
				to={`/post/${postId}`}
				className='bg-accent mt-2 p-2 rounded-md flex items-center space-x-2 hover:bg-gray-100 transition-colors'
			>
				{relatedPost.image && (
						...
						<ul>
							{notifications.data.map((notification) => (
								<li
									...
									className={`bg-accent text-info border rounded-lg p-4 my-4 transition-all hover:shadow-md  focus:outline-bg-primary${
										!notification.read ? "border-red-500" : "border-green-500"
									}`}
								>
										...
										<div className='flex gap-2'>
											{!notification.read && (
												<button
													...
													className='p-1 text-green-600 rounded hover:bg-blue-200 transition-colors'
